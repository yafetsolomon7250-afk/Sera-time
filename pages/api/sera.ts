...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    // health without auth
    if (action === "health") {
      return ok({ ok: true, service: "sera-time", version: "10.0", time: new Date().toISOString() });
    }
...
switch (action) {
      ...
      case "referrals": {
        ...
        const { data: list } = await db()
          .from("users")
          .select("id,first_name,username,created_at,telegram_id")
        ...
        return ok({
          ...dash,
          link,
          count: (list || []).length || dash?.count || 0,
      ...
      case "claim-ad": {
        ensureWorker(user);
        const limit = num(process.env.ADS_DAILY_LIMIT, 5);
        const reward = num(process.env.ADS_REWARD_ETB, 2);
        ...
        if ((count || 0) >= limit) throw new Error("ዛሬ ያለው ማስታወቂያ ገደብ ተሟልቷል።");
        // credit wallet
        const { data: w } = await db().from("wallets").select("available").eq("user_id", user.id).maybeSingle();
        if (!w) await db().from("wallets").insert({ user_id: user.id, available: reward, locked: 0 });
        else await db().from("wallets").update({ available: Number(w.available || 0) + reward }).eq("user_id", user.id);
        await db().from("ad_rewards").insert({ user_id: user.id, amount: reward, provider: "adsgram" });
        await db().from("ledger_entries").insert({
          user_id: user.id,
          type: "ad_reward",
          amount: reward,
        ...
        // small XP
        const { data: wu } = await db().from("users").select("xp").eq("id", user.id).maybeSingle();
        if (wu) {
          const xp = (Number(wu.xp) || 0) + 5;
          await db().from("users").update({ xp, level: Math.floor(xp / 200) + 1 }).eq("id", user.id);
        }
        return ok({ ok: true, reward, remaining: limit - (count || 0) - 1 });
      }
