...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    // health without auth
    if (action === "health") {
      return ok({ ok: true, service: "sera-time", version: "11.0", time: new Date().toISOString() });
    }

    const user = await authUser(String(body.initData || ""), body.ref, body.guestId);
...
switch (action) {
      ...
      case "dashboard": {
        const d = await rpc("dashboard", { p_user_id: user.id, p_role: body.role || user.active_role || "worker" });
        return ok({ ...d, xp: user.xp || 0, level: user.level || 1, streak: user.streak_days || 0 });
      }
      ...
      case "my-tasks": {
        ensureWorker(user);
        const raw = await rpc("my_tasks", { p_worker_id: user.id });
        const tasks = [];
        for (const t of raw || []) {
          const { data: files } = await db().from("task_files").select("id,file_path,file_url,file_name,mime_type,size_bytes,created_at").eq("task_id", t.id).order("created_at", { ascending: true });
          const { data: client } = await db().from("users").select("first_name,username").eq("id", t.client_id).maybeSingle();
          const signedFiles = [];
          for (const f of files || []) {
            let url = f.file_url || null;
            if (f.file_path) { try { const { data: signed } = await db().storage.from("sera-time-files").createSignedUrl(f.file_path, 3600); url = signed?.signedUrl || url; } catch {} }
            signedFiles.push({ ...f, url });
          }
          tasks.push({ ...t, client_name: client?.first_name || client?.username || "Sera Time Client", client_username: client?.username || null, files: signedFiles });
        }
        return ok({ tasks });
      }
      ...
      case "referrals": {
        ...
        const link = `https://t.me/${bot}?start=r_${encodeURIComponent(String(user.telegram_id))}`;
        let dash: any = {};
        try {
          dash = await rpc("referral_dashboard", { p_user_id: user.id, p_bot_username: bot });
        } catch {}
        // invited list comes from the referrals table (users has no referred_by column)
        ...
        if (referredIds.length) {
          ...
          for (const u of invitedUsers || []) byId[u.id] = u;
          list = (refs || []).map((r: any) => ({ ...byId[r.referred_id], referral_id: r.id, referred_at: r.created_at }));
        }
        // top 3 inviters by count — referrals is the source of truth.
        // Do not read users.referred_by because V11 stores referral relationships
        ...
        if (topIds.length) {
          const { data: tops } = await db()
            ...
            .in("id", topIds);
          topInviters = (tops || [])
            .map((u: any) => ({ ...u, invite_count: counts[u.id] || 0, earned: 0 }))
            .sort((a: any, b: any) => b.invite_count - a.invite_count);
        }
        return ok({
          ...dash,
          link,
          count: (list || []).length || Number(dash?.count || 0),
          earned: dash?.earned || 0,
          list: list || [],
          topInviters,
        });
      }
      ...
      case "create-deposit": {
        ...
        if (amount < 100) throw new Error("ዝቅተኛ ዲፖዚት 100 ብር ነው።");
        if (!body.proofUrl && !body.proofPath) throw new Error("የክፍያ ስክሪንሹት ያስገቡ።");
        // Use security-definer RPC (has table permission)
        try {
          await rpc("create_deposit", {
            p_user_id: user.id,
            p_amount: amount,
            p_method: "telebirr",
          ...
          // if RPC missing, show clear error
          throw new Error(e?.message || "ዲፖዚት አልተሳካም");
        }
        // Always re-read the row so we have a real UUID for buttons
        const { data: deposit } = await db()
          ...
          .limit(1)
          .maybeSingle();
        if (!deposit?.id) throw new Error("ዲፖዚት ተፈጥሯል ግን ID አልተገኘም። RLS/GRANT ይፈትሹ።");
      ...
      case "withdraw": {
        ...
        let r: any = await rpc("create_withdrawal", {
          p_user_id: user.id,
          p_amount: amount,
          p_method: method,
        ...
        if (!r?.id) throw new Error("ማውጣት መፍጠር አልተሳካም።");
      ...
      case "admin-withdraw": {
        ...
        const { data: wd } = await db().from("withdrawals").select("id,user_id,amount,status").eq("id", id).maybeSingle();
        if (!wd) throw new Error("ማውጣት አልተገኘም።");
        if (wd.status !== "pending") throw new Error("አስቀድሞ ተስተናግዷል።");
        const { data: wal } = await db().from("wallets").select("available,reserved").eq("user_id", wd.user_id).maybeSingle();
        if (approve) {
          if (wal) await db().from("wallets").update({ reserved: Math.max(0, Number(wal.reserved || 0) - Number(wd.amount)) }).eq("user_id", wd.user_id);
          await db().from("ledger_entries").insert({ user_id: wd.user_id, type: "withdrawal", amount: -Number(wd.amount), description: "Withdrawal paid", reference_id: id });
          await db().from("withdrawals").update({ status: "paid", processed_by: user.id, processed_at: new Date().toISOString() }).eq("id", id);
        } else {
          if (wal) await db().from("wallets").update({ available: Number(wal.available || 0) + Number(wd.amount), reserved: Math.max(0, Number(wal.reserved || 0) - Number(wd.amount)) }).eq("user_id", wd.user_id);
          await db().from("withdrawals").update({ status: "rejected", processed_by: user.id, processed_at: new Date().toISOString(), rejection_reason: "Rejected by admin" }).eq("id", id);
        }
        await notify(wd.user_id, approve ? "ማውጣት ተከፍሏል" : "ማውጣት ተቀባይነት አላገኘም",
          approve ? "የማውጣት ጥያቄዎ ተከፍሏል።" : "የማውጣት ጥያቄዎ ተቀባይነት አላገኘም።", "withdrawal");
        await tgMessage(wd.user_id, approve ? "✅ ማውጣት ተከፍሏል።" : "❌ ማውጣት ተቀባይነት አላገኘም።");
        return ok({ ok: true, paid: approve });
      }
      ...
      case "admin-deposit": {
        ...
        const { data: d } = await db().from("deposits").select("id,user_id,amount,status").eq("id", id).maybeSingle();
        if (!d) throw new Error("ዲፖዚት አልተገኘም።");
        if (d.status !== "pending") throw new Error("አስቀድሞ ተስተናግዷል።");
        if (approve) {
          const { data: w } = await db().from("wallets").select("available").eq("user_id", d.user_id).maybeSingle();
          if (!w) await db().from("wallets").insert({ user_id: d.user_id, available: Number(d.amount), locked: 0 });
          else await db().from("wallets").update({ available: Number(w.available || 0) + Number(d.amount) }).eq("user_id", d.user_id);
          await db().from("ledger_entries").insert({ user_id: d.user_id, type: "deposit", amount: Number(d.amount), description: "Deposit approved", reference_id: id });
          await db().from("deposits").update({ status: "approved", reviewed_by: user.id, reviewed_at: new Date().toISOString() }).eq("id", id);
        } else {
          await db().from("deposits").update({ status: "rejected", reviewed_by: user.id, reviewed_at: new Date().toISOString() }).eq("id", id);
        }
        ...
        return ok({ ok: true, approved: approve });
      }
