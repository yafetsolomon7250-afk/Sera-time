...
async function welcome(chatId: number | string, startParam = "") {
  const baseApp = process.env.APP_URL || "https://sera-time-zhyo.vercel.app";
  const app = startParam ? `${baseApp}${baseApp.includes("?") ? "&" : "?"}ref=${encodeURIComponent(startParam)}` : baseApp;
  const keyboard = {
    inline_keyboard: [
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (req.method === "GET") {
      return res.status(200).json({ ok: true, service: "telegram-webhook", version: "11.0" });
    }
    ...
    if (cb?.data && cb?.from?.id) {
      ...
      if (isDep || (!isWd && msgText.includes("ዲፖዚት"))) {
        ...
        if (cb.message?.chat?.id && cb.message?.message_id) await deleteMessage(cb.message.chat.id, cb.message.message_id);
    ...
    if (msg?.chat?.id) {
      const startParam = String(msg.text || "").match(/^\/start(?:\s+(.+))?$/i)?.[1] || "";
      await welcome(msg.chat.id, startParam);
    }
    return res.status(200).json({ ok: true });
  } catch (e: any) {
    console.error(e);
