...
`https://YOUR-VERCEL-DOMAIN/api?action=ads-reward&userid=[userId]`
...
\n## Minimal structure\nOnly two main folders are used: `app/` and `public/`. The server endpoints are consolidated into one dynamic API route.\n

## API structure
All API endpoints are intentionally consolidated into `app/api/route.ts` to keep the project small and avoid dynamic-route typing issues on Next.js 16.
