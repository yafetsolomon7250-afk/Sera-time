...
function Tasks({ data, reload }: any) {
      ...
      <div className="skillFull">
        <div className="skillHeader">
          ...
          <div>
            ...
            <small style={{ color: "var(--muted)" }}>{filtered.length} ስራ ይገኛል</small>
          </div>
        </div>
        ...
        {!filtered.length && <Empty text={`${activeCat.label} ላይ አሁን ስራ የለም`} />}
      </div>
    );
  }
  ...
  return (
    <div className="stack">
      ...
      {!catKey && query && filtered.map((t: any) => (
        <TaskCard key={t.id} t={t} reload={reload} />
      ))}
      {!catKey && !query && (
        <Empty text="ከላይ ችሎታ በመምረጥ ስራዎችን ይመልከቱ" />
      )}
      {!catKey && query && !filtered.length && <Empty text="የፈለጉት ስራ አልተገኘም" />}
    </div>
  );
}
