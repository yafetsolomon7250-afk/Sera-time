...
async function api(action: string, body: any = {}) {
  ...
  const r = await fetch(`/api/sera`, {
    ...
    body: JSON.stringify({ ...body, action, initData, ref }),
  });
...
function Invite({ data }: any) {
  ...
  const shareText = `Sera Time ላይ ስራ ሰርተህ ገቢ አግኝ! 🇪🇹\n${link}`;
  const copy = async () => {
    try {
  ...
  return (
    <div className="stack">
      ...
      <div className="inviteHero inviteCard">
        <div className="heroMark" style={{ margin: "0 auto 10px" }}>♧</div>
        <b>የግብዣ ሊንክዎ</b>
        <p>የተጋበዘ ሰራተኛ ስራ ካጠናቀቀ እና ክፍያ ከተፈጸመ በኋላ <b>5%</b> ይገባዎታል።</p>
        <div className="inviteLink linkBox">{link || "-"}</div>
        <div className="actions" style={{ justifyContent: "center", gap: 8, flexWrap: "wrap" }}>
          <button onClick={copy}>📋 ቅዳ</button>
          <button onClick={shareTg}>✈️ ቴሌግራም አጋራ</button>
        </div>
      </div>
      ...
      {(data.list || []).length > 0 && (
        <div className="card">
          <h3>የቅርብ ግብዣዎች</h3>
          {(data.list || []).slice(0, 20).map((r: any, i: number) => (
            <div className="adminRow" key={i}>
              <div><b>{r.first_name || r.name || "ተጠቃሚ"}</b><small>{r.created_at ? new Date(r.created_at).toLocaleDateString() : ""}</small></div>
