  ...
  try {
    ...
    switch (action) {
      ...
      case "referrals": {
        ...
        const link = `https://t.me/${bot}/Sera?startapp=ref_${user.id}`;
      ...
      case "admin-adjust-balance": {
        if (!user.is_admin) throw new Error("ፈቃድ የለዎትም።");
        return ok(await rpc("admin_adjust_balance", {
          p_admin_id: user.id,
          p_user_id: text(body.userId, 80),
      ...
      case "admin-broadcast": {
        ...
        const { data: users } = await db().from("users").select("id,telegram_id").not("telegram_id", "is", null).limit(500);
        let sent = 0;
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ action: string }> }) {
  try {
    const { action } = await params;
    if (action === "health") {
      return ok({ ok: true, service: "sera-time", version: "8.0", time: new Date().toISOString() });
    }
    if (action !== "ads-reward") return fail("Not found", 404);
    ...
    if (!telegramId) return fail("userid required", 400);
    const { data: u } = await db().from("users").select("id,banned").eq("telegram_id", telegramId).maybeSingle();
    if (!u || u.banned) return fail("not eligible", 403);
