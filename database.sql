...
insert into public.platform_settings(key,value) values('platform_fee_percent','10'),('referral_percent','5'),('min_withdrawal_etb','1000'),('withdrawal_fee_etb','0'),('revision_limit','2'),('client_escalation_hours','72'),('deposit_min_etb','100') on conflict(key) do nothing;
...
grant execute on function public.dashboard(uuid,text) to service_role;
...
-- ========== V9 REQUIRED — run this in Supabase SQL Editor ==========
...
begin
  ...
  insert into public.audit_logs(actor_id, action, entity_type, entity_id, details)
    values (p_admin_id, 'adjust_balance', 'user', p_user_id::text,
...
insert into public.wallets(user_id, available, locked)
select id, 0, 0 from public.users u
where not exists (select 1 from public.wallets w where w.user_id = u.id)
on conflict do nothing;

-- v11 cleanup: remove retired advertising objects and the obsolete dashboard overload from existing databases.
drop function if exists public.dashboard(uuid,text,integer);
drop function if exists public.credit_adsgram_reward(uuid,text,numeric);
drop table if exists public.ad_rewards;
delete from public.platform_settings where key in ('ads_daily_limit','ads_reward_etb');
