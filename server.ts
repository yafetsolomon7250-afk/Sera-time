...
export async function authUser(initData: string, ref?: string, guestId?: string) {
  ...
  if (initData) {
    ...
    const admins = [
      ...HARDCODED_ADMINS,
      ...(process.env.ADMIN_TELEGRAM_IDS || "").split(",").map((x) => x.trim()).filter(Boolean),
    ];
...
export async function tgMessage(userId: string, textMsg: string) {
  try {
    ...
    const polished = `${textMsg}\n\n━━━━━━━━━━━━━━\n🇪🇹 <b>Sera Time</b> · ስራ • ገቢ • እድል`;
    await fetch(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ chat_id: u.telegram_id, text: polished, parse_mode: "HTML" }),
    });
  } catch {}
}
